...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    function key(event) {
      ...
      if (event.path === 'pr' && event.target && typeof event.target.repo === 'string' && Number.isSafeInteger(event.target.number)) return `pr:${event.target.repo}#${event.target.number}`
      return undefined
    }
    function targetFor(marker) {
      ...
      const agentId = marker.agentId === undefined ? selectedAgentId : marker.agentId
      if (agentId === undefined) return undefined
      return [...document.querySelectorAll(".canvas-node")].find(node => node.dataset.agentId === agentId)
    }
    ...
    function receivePath(event) {
      ...
      if (!detail || (detail.path !== 'pr' && detail.path !== 'bot')) return
      const markerKey = key(detail); if (!markerKey) return
      const existing = markers.find(marker => key(marker) === markerKey); if (existing) existing.hasPath = true; else markers.push({ path: detail.path, target: detail.target, agentId: detail.path === "pr" ? selectedAgentId : undefined, hasPath: true }); drawMarkers()
    }
    function receiveSent(event) {
      const detail = event.detail
      if (!detail || (detail.path !== 'pr' && detail.path !== 'bot')) return
      const markerKey = key(detail); if (!markerKey) return
      if (!markers.some(marker => key(marker) === markerKey)) markers.push({ path: detail.path, target: detail.target, agentId: detail.path === "pr" ? selectedAgentId : undefined, hasPath: false }); drawMarkers()
    }
    function draw(event) {
      lastEvent = event
    ...
    function select(event) { const agentId = event.detail?.agentId; if (agentId === undefined) throw new Error('connect: node selection has no agentId'); selectedAgentId = agentId; if (lastEvent) drawMarkers() }
