...
.canvas-surface{position:relative;flex:1;min-height:0;overflow:auto;background:#07080a}.canvas-plane{position:relative;background:#07080a}.canvas-layer{position:absolute;inset:0}.canvas-groups{z-index:1}.canvas-edges{z-index:1;pointer-events:none}.canvas-nodes{z-index:2}.canvas-origin{position:absolute;z-index:3;width:320px;height:320px;transform:translate(-50%,-50%);pointer-events:none}.canvas-orbit{position:absolute;inset:50%;transform:translate(-50%,-50%);border:1px dashed #171c22;border-radius:50%}.canvas-orbit-outer{width:320px;height:320px;border-color:#101419}.canvas-orbit-mid{width:208px;height:208px}.canvas-orbit-inner{width:144px;height:144px;border-color:#1d2732}.canvas-core{position:absolute;left:50%;top:50%;width:72px;height:72px;transform:translate(-50%,-50%);border-radius:50%;background:#111923;box-shadow:0 0 0 16px #111923,0 0 0 32px #0c121a,0 0 36px #2b5d9b66}.canvas-core:before{content:"";position:absolute;inset:16px;border-radius:50%;background:#1d3553}.canvas-core:after{content:"";position:absolute;left:50%;top:50%;width:12px;height:12px;transform:translate(-50%,-50%);border-radius:50%;background:#f5f8ff;box-shadow:0 0 10px 3px #8ec4ff}.canvas-live{position:absolute;z-index:4;left:19px;top:16px;width:48px;height:80px;display:flex;align-items:center;justify-content:center;border:1px solid #242728;border-radius:24px;background:#101215;color:#d9dde5}.canvas-live span{display:flex;align-items:center;gap:6px}.canvas-live i{width:6px;height:6px;border-radius:50%;background:#35e88a}.canvas-note{position:absolute;z-index:4;left:19px;bottom:16px;color:#626b79;font-size:11px}.canvas-status{position:absolute;z-index:4;left:24px;top:24px;color:#8f99a8;font-weight:600}
`
...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    const CSS = `.canvas-node{position:absolute;z-index:2;box-sizing:border-box;width:134px;height:46px;padding:9px 11px;border:1px solid #242728;border-radius:8px;background:#0e1014;color:#d9dde5;cursor:pointer;box-shadow:0 6px 22px #0008;transition:border-color .15s,box-shadow .15s}.canvas-node:hover{border-color:#52647d;box-shadow:0 8px 28px #000b}.canvas-node[data-status="running"]{border-color:#385f87}.canvas-node[data-status="failed"]{border-color:#713d43}.canvas-node-name{font-weight:700;line-height:15px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.canvas-node-meta{margin-top:4px;color:#656e7b;font-size:10px;display:flex;justify-content:space-between;gap:12px}.canvas-node-report{position:absolute;right:5px;top:5px;width:18px;height:18px;border:0;border-radius:4px;background:transparent;color:#697483;cursor:pointer}.canvas-node-report:hover{background:#1a1e24;color:#d9dde5}.canvas-node-unread{position:absolute;right:6px;top:6px;width:6px;height:6px;border-radius:50%;background:#ff5d63;display:none;box-shadow:0 0 0 2px #0e1014}.canvas-node[data-unread="true"] .canvas-node-unread{display:block}.canvas-dot{width:6px;height:6px;border-radius:50%;display:inline-block;margin-right:5px;background:#777f8c}.canvas-dot[data-status="running"]{background:#4aa3ff}.canvas-dot[data-status="done"]{background:#35e88a}.canvas-dot[data-status="failed"]{background:#ff5d63}.canvas-dot[data-status="waiting"]{background:#a6adb7}.canvas-dot[data-status="idle"]{background:#777f8c}.canvas-group{position:absolute;z-index:1;border:1px solid #242728;background:#0e101488;border-radius:10px;cursor:pointer}.canvas-group-label{position:absolute;top:8px;left:10px;color:#626b79;font-size:10px;font-weight:700}.canvas-inspector{position:absolute;z-index:4;right:19px;top:19px;width:224px;box-sizing:border-box;padding:12px;border:1px solid #242728;border-radius:11px;background:#0e1014;color:#d9dde5;box-shadow:0 8px 24px #0008}.canvas-inspector-title{font-weight:700;font-size:12px}.canvas-inspector-subtitle{margin-top:6px;color:#656e7b;font-size:10px}.canvas-inspector-callout{margin-top:10px;padding:9px;border:1px solid #242728;border-radius:8px;color:#d0d6df;font-size:10px;line-height:14px}.canvas-inspector-callout small{display:block;margin-top:3px;color:#656e7b}`
    ...
    function layout(snapshot, origin) {
      ...
      const roots = new Set(snapshot.roots); const agents = [...snapshot.agents].sort((a, b) => Number(roots.has(b.id)) - Number(roots.has(a.id))); const radius = Math.min(220, Math.max(150, Math.min(origin.x, origin.y) * .58)); const result = new Map()
      agents.forEach((agent, index) => { const angle = index === 0 ? -Math.PI / 2 : -Math.PI / 2 + (index % 2 === 1 ? -1 : 1) * Math.ceil(index / 2) * Math.PI / 3; result.set(agent.id, { x: origin.x + Math.cos(angle) * radius - 67, y: origin.y + Math.sin(angle) * radius - 23, width: 134, height: 46 }) })
      return result
    }
